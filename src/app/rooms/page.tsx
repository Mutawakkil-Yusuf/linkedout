  ...
  return (
    <div className="pt-8">
      ...
      <ul className="space-y-2.5">
        {rooms?.length ? rooms.map((r) => <li key={r.id}><Link href={`/rooms/${r.slug}`} className="block rounded-card border border-line bg-card p-5 transition hover:border-line-2"><div className="flex items-center gap-2"><span className="font-mono text-flame">#</span><span className="font-display text-[1.05rem] font-bold tracking-[-0.02em]">{r.slug}</span></div><p className="mt-0.5 text-[0.9rem] text-muted">{r.name}</p>{r.description && <p className="mt-2 text-[0.9rem] leading-relaxed text-ink-2">{r.description}</p>}</Link></li>) :
        <li className="rounded-card border border-line bg-card p-6 text-center text-[0.95rem] text-muted">No rooms yet. Create one.</li>}
      </ul>
    </div>
  );
}
