...
export default async function PostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="pt-8"><PostCard post={post as any} author={(post as any).author} /><ReplyForm postId={id} /><ul>{replies?.map((r: any) => <li key={r.id} className="mb-3 rounded-card border border-line bg-card p-4"><header className="mb-2 flex items-center gap-2.5"><Avatar handle={r.author?.handle ?? "??"} size="sm" /><div className="text-[0.9rem] font-semibold">{r.author?.display_name ?? r.author?.handle}</div><time className="ml-auto font-mono text-[0.72rem] text-muted">{relativeTime(r.created_at)}</time></header><p className="whitespace-pre-wrap text-[1rem] leading-relaxed text-ink">{r.body}</p></li>)}</ul></div>;
}
