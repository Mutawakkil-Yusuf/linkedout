...
export default async function PostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: post } = await supabase.from("posts").select("id, body, created_at, author_id, room_id, author:profiles!posts_author_id_fkey(handle, display_name, avatar_style, avatar_seed), reactions(count), room:room_id(slug, accent)").eq("id", id).maybeSingle();
  if (!post) notFound();
