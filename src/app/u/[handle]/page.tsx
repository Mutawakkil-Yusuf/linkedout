...
export default async function ProfilePage({ params }: { params: Promise<{ handle: string }> }) {
  ...
  return <div className="pt-8"><header className="flex items-center gap-4 border-b border-line pb-5"><Avatar handle={profile.handle} size="lg" /><div className="min-w-0"><h1 className="break-words font-display text-[1.4rem] font-bold tracking-[-0.025em]">{profile.display_name ?? `@${profile.handle}`}</h1><div className="mt-0.5 break-words font-mono text-[0.8rem] text-muted">@{profile.handle} · here since {joined}</div></div></header><NowLine text={profile.now} />{profile.bio && <p className="mb-3 whitespace-pre-wrap break-words text-[0.95rem] leading-relaxed text-ink-2">{profile.bio}</p>}<ul className="mt-6">{posts?.length ? posts.map((p: any) => <li key={p.id}><PostCard post={p} author={p.author} /></li>) : <li className="rounded-card border border-line bg-card p-6 text-center text-[0.95rem] text-muted">Nothing posted yet.</li>}</ul></div>;
}
