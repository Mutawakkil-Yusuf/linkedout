...
export default async function Landing() {
  ...
  return (
    <div className="mx-auto max-w-[44rem] px-4 py-10 sm:px-5 sm:py-14">
      <header className="flex flex-wrap items-center justify-between gap-x-4 gap-y-3">
        <Lockup size={34} />
        <Link href="/login" className="font-mono text-[0.72rem] text-muted underline decoration-line underline-offset-4 hover:text-ink sm:text-[0.78rem]">
      ...
      <section className="pt-12 pb-12 sm:pt-20 sm:pb-16">
        ...
        <h1 className="mb-6 text-balance font-display text-[2.15rem] font-bold leading-[1.05] tracking-[-0.03em] sm:text-[2.6rem] sm:leading-[1.03] md:text-[3.15rem] md:leading-[1.02] md:tracking-[-0.04em]">
        ...
        <p className="mb-10 max-w-[34rem] text-[1rem] leading-relaxed text-ink-2 sm:text-[1.125rem]">
      ...
      <section id="rooms" className="py-16">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {rooms?.length ? rooms.map((r) => (
            <div key={r.slug} className="rounded-card border border-line bg-card p-5">
              <div className="mb-1 flex min-w-0 items-center gap-2"><span className="shrink-0 font-mono text-flame">#</span><span className="truncate font-display text-[1.05rem] font-bold tracking-[-0.02em]">{r.slug}</span></div>
              <p className="break-words text-[0.88rem] text-muted">{r.name}</p>
              {r.description && <p className="mt-2 break-words text-[0.9rem] leading-relaxed text-ink-2">{r.description}</p>}
            </div>
          )) : [
            ...
            <div key={r.slug} className="rounded-card border border-line bg-card p-5">
              <div className="mb-1 flex items-center gap-2"><span className="font-mono text-flame">#</span><span className="font-display text-[1.05rem] font-bold tracking-[-0.02em]">{r.slug}</span></div>
      ...
      <section className="border-t border-line pt-14">
        <h2 className="mb-6 max-w-[28rem] text-balance font-display text-[1.6rem] font-bold leading-[1.15] tracking-[-0.025em] sm:text-[2rem] sm:leading-[1.1] sm:tracking-[-0.03em]">You are not inventory. Come be a person for a while.</h2>
